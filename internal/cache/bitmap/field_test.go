...
func TestFieldOps_BitField(t *testing.T) {
	// Create mock sync.Maps
	cache := &sync.Map{}
	version := &sync.Map{}
	...
	t.Run("Test BitField with Invalid Command", func(t *testing.T) {
		commands := []models.BitFieldCommand{
			{Op: "INVALID", Type: "u4", Offset: 0}, // Invalid command
		}

		_, err := fieldOps.BitField(key, commands)
		assert.Error(t, err, "BitField should return an error for invalid commands")
		assert.EqualError(t, err, "ERR unknown bitfield command INVALID")
	})
}
